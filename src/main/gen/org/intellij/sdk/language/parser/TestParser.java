...
public class TestParser implements PsiParser, LightPsiParser {
  ...
  static boolean parse_root_(IElementType t, PsiBuilder b, int l) {
    return TesboFile(b, l + 1);
  }

  /* ********************************************************** */
  // item_*
  static boolean TesboFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "TesboFile")) return false;
    while (true) {
      int c = current_position_(b);
      if (!item_(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "TesboFile", c)) break;
    }
    return true;
  }
